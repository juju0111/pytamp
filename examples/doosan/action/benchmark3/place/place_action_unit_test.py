...
eef_poses = list(pick.get_all_grasp_poses("rect_box1"))

# Add heuristic
eef_poses.extend(list(pick.get_grasp_pose_from_heuristic("rect_box1")))

all_release_poses = []
for eef_pose in eef_poses:
    release_poses = list(
        place.get_all_release_poses_and_obj_pose(
            "tray_blue", "rect_box1", eef_pose[pick.move_data.MOVE_grasp]
        )
    )
