...
def main():
    ...
    ################# Action Test ##################
    rearr_actions = list(ArrangementAction.get_possible_actions_level_1(scene_for_sample=rearrangement1.init_scene))
    fig, ax = p_utils.init_3d_figure(name="Rearrangement 1")
    for rearr_action in rearr_actions:
        for rearr_scene in ArrangementAction.get_possible_transitions(scene=ArrangementAction.scene_mngr.scene, action=rearr_action):
            # init_scene
