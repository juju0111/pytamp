...
class PlaceAction(ActivityBase):
        ...
        for (
            ...
            for held_obj_point, held_obj_normal in surface_points_for_held_obj:
                ...
                if eef_pose is not None:
                    T_obj_pose_and_obj_pose_transformed = np.dot(
                        held_obj_pose, np.linalg.inv(held_obj_pose_rotated)
                    )
                    eef_pose_transformed = self._get_eef_pose_transformed(
                        T_obj_pose_and_obj_pose_transformed,
                        eef_pose,
                        support_obj_point,
                        held_obj_point_transformed,
                    )
    ...
    def _get_obj_pose_transformed(
        ...
        obj_pose_transformed[:3, 3] = held_obj_pose[:3, 3] + (
            sup_obj_point - held_obj_point_transformed
        )
    ...
    def _get_eef_pose_transformed(
        ...
        result_eef_pose_transformed[:3, 3] = (
            eef_pose_transformed[:3, 3]
            + (sup_obj_point - held_obj_point_transformed)
            + np.array([0, 0, self.release_distance])
        )
