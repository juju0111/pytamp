...
class Benchmark3_for_rearr(Benchmark):
    ...
    def _load_objects(self):
        ...
        for i in range(self.square_box_num):
            self.square_boxes.append(get_object_mesh("ben_cube.stl", [0.05, 0.05, 0.07]))
        ...
        for i in range(self.rect_box_num):
            self.rect_boxes.append(get_object_mesh("rect_box.stl", [0.00175, 0.000525, 0.00175]))
        ...
        self.milk1 = get_object_mesh("milk.stl", [1.3, 1.3, 1.3])
        self.milk1.apply_translation(-self.milk1.center_mass)

        self.milk2 = get_object_mesh("milk.stl", [1.3, 1.3, 1.3])
        self.milk2.apply_translation(-self.milk2.center_mass)

        self.milk3 = get_object_mesh("milk.stl", [1.3, 1.3, 1.3])
        self.milk3.apply_translation(-self.milk3.center_mass)

        self.milk4 = get_object_mesh("milk.stl", [1.3, 1.3, 1.3])
        self.milk4.apply_translation(-self.milk4.center_mass)

        self.milk5 = get_object_mesh("milk.stl", [1.3, 1.3, 1.3])
        self.milk5.apply_translation(-self.milk4.center_mass)

        self.can = get_object_mesh("can.stl", [1.3, 1.3, 2.0])
        ...
        self.can_pose = Transform(
            pos=np.array(
                [
                    0.72,
                    0.4,
                    self.table_height + self.clearbox_8_height + abs(self.can.bounds[0][2]),
                ]
            )
        )
        self.tray_blue_pose = Transform(pos=np.array([0.6, 0, 0.745]))
    ...
    def make_scene(self):
        ...
        for name in self.scene_mngr.scene.objs:
            if self.scene_mngr.scene.logical_states[name].get(
                self.scene_mngr.scene.logical_state.static
            ):
                if name == "clearbox":
                    object_meshes.append(self.scene_mngr.scene.objs[name].gparam)
                    object_names.append(name)
                    object_poses.append(
                        deepcopy(self.scene_mngr.scene.objs[name].h_mat)
                        @ np.linalg.inv(self.scene_mngr.scene.objs["table"].h_mat)
                    )
                    continue
                elif name == "tray_blue":
                    object_meshes.append(self.scene_mngr.scene.objs[name].gparam)
                ...
                support_meshes.append(self.scene_mngr.scene.objs[name].gparam)
                support_names.append(name)

            else:
                object_meshes.append(self.scene_mngr.scene.objs[name].gparam)
                object_names.append(name)
                object_color[name] = self.scene_mngr.scene.objs[name].color
                object_poses.append(
                    deepcopy(self.scene_mngr.scene.objs[name].h_mat)
                    @ np.linalg.inv(self.scene_mngr.scene.objs["table"].h_mat)
                )
