...
class Benchmark4_for_rearr(Benchmark):
    def __init__(self, robot_name="panda", disk_num=3, geom="visual", is_pyplot=True):
    ...
    def _load_objects(self):
        ...
        self.peg2_pose = Transform(
            pos=np.array([0.7, 0, self.table_height + self.peg_mesh_bound[1][2]])
        )
        self.peg3_pose = Transform(
            pos=np.array([0.7, -0.30, self.table_height + self.peg_mesh_bound[1][2]])
        )
    ...
    def _load_scene(self):
        self.scene_mngr.add_object(
            name="peg_1",
            gtype="mesh",
            gparam=self.peg_mesh,
            h_mat=self.peg1_pose.h_mat,
            color=[0.0, 0.0, 1.0],
        )
        self.scene_mngr.add_object(
            name="peg_2",
        ...
        for i in range(self.disk_num):
            disk_pos = np.array(
                [
                    0.69,
                    0.3,
                    self.table_height + self.disk_mesh_bound[1][2] + self.disk_heigh * i,
                ]
            )
            ...
            self.disk_mesh.append(hanoi_mesh)

            self.disk_color[disk_name] = [0.0, 1.0, 0.0]

            self.scene_mngr.add_object(
                name=disk_name,
                gtype="mesh",
                gparam=hanoi_mesh,
                h_mat=self.disk_pose[i].h_mat,
                color=[0.0, 1.0, 0.0],
            )
        ...
        for i in range(self.disk_num):
            ...
            if disk_name == "hanoi_disk_0":
                ...
                self.scene_mngr.set_logical_state(
                    disk_name, ("on", prev_disk_name), ("hang", "peg_1")
                )
    ...
    #     # for disk
    #     for i in range(self.disk_num):
    #         object_meshes.append(self.disk_mesh[i])
    #         object_names.append(self.disk_name[i])
    #         support_meshes.append(self.disk_mesh[i])
    #         support_names.append(self.disk_name[i] + "_support")
    #         object_color[self.disk_name[i]] = self.disk_color[i]
    #         sup_obj_name = (
    #             self.scene_mngr.scene.logical_states[self.disk_name]
    #             .get(self.scene_mngr.scene.logical_state.on)
    #             .name
    #         )
    #         object_poses.append(
    #             self.disk_pose[i].h_mat
